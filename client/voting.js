...
const postToAccordion = (data) => {
  ...
  data.forEach((element) => {
    ...
    if (data[counter]["submission_status"] == "approved") {
      ...
      modalBody.innerHTML = `Cast your Vote! You have ${
        7 - localStorage.getItem("votes")
      } votes remaining.`;
      ...
      voteButtonModal.setAttribute(
        "proposalId",
        data[counter]["submission_id"]
      );
...
const castVote = async (proposal_id) => {
  ...
  try {
    const res = await fetch(
      `http://localhost:3000/submissions/vote/${proposal_id}`,
      options
    );
    if (res.ok) {
      const data = await res.json();
